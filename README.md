# 👋 Bonjour, je suis Joel Prince Yehezekocolo

![Bannière](https://via.placeholder.com/1500x300/1E293B/FFFFFF?text=Joel+Prince+Yehezekocolo)

## 👨‍💻 Développeur Fullstack

Passionné par la création d'expériences numériques à fort impact. Je conçois et développe des applications web et mobiles modernes, performantes et esthétiques pour répondre aux besoins spécifiques de mes clients.

## 🛠️ Mes compétences techniques

- **Flutter**: Création d'applications mobiles fluides et multiplateformes avec des interfaces utilisateur soignées.

- **Express Js**: Développement d'APIs RESTful robustes et conception de systèmes backend sécurisés avec bases de données.

- **Svelte/Angular**: Construction d'applications web complexes et évolutives avec une gestion avancée des états.

- **Electron Js**: Réalisation d'applications de bureau performantes compatibles avec tous les systèmes d'exploitation.

## 🧰 Mes outils préférés

- **Éditeurs**: VS Code & Android Studio
- **Gestion de versions**: Git & GitHub
- **Conteneurisation**: Docker
- **Design UI/UX**: Figma

## 📫 Me contacter

- **Email**: joelprinceyeheze@gmail.com
- **LinkedIn**: [Joel Prince Yehezekocolo](https://bj.linkedin.com/in/joel-prince-yehezekocolo-b2700a352)
- **Localisation**: Porto-Novo, Bénin
- **Disponibilité**: Lun-Sam, 9h-18h

## 📊 Mes statistiques GitHub

![Statistiques GitHub](https://github-readme-stats.vercel.app/api?username=Jolyeh&show_icons=true&theme=tokyonight) ![Langages les plus utilisés](https://github-readme-stats.vercel.app/api/top-langs/?username=Jolyeh&layout=compact&theme=tokyonight)

## 💼 Expérience

Je suis curieux, rigoureux et toujours en quête d'apprentissage. Je crois que la technologie doit avant tout être au service des gens. C'est pourquoi je m'investis pleinement dans chaque ligne de code, avec le souci du détail et l'envie constante de faire mieux.

---

💬 N'hésitez pas à me contacter pour discuter de vos projets ou pour toute opportunité de collaboration!
